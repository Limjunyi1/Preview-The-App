...
class TestTopTradingCycle(TestCase):
    ...
    def test_blocking_pairs(
        ...
        blocking_pairs = find_blocking_pairs(
            one_sided_match, two_sided_match, employee_preferences, job_preferences
        )
    ...
    def test_basic_graph_methods(
        ...
        self.assertSetEqual(all_nodes, set(["e1", "e2", "j1", "j2"]))
        self.assertEqual(G.number_of_nodes_in_graph(), 4)
        # check assertions after Graph update
        G.delete_node("e1")
        self.assertSetEqual(set(G.nodes.keys()), set(["e2", "j1", "j2"]))
        self.assertEqual(G.number_of_nodes_in_graph(), 3)
        # add node back in
        G.add_node("e1")
        ...
        self.assertSetEqual(
            G.edges, {("e1", "j1"), ("j1", "e2"), ("e2", "j2"), ("j2", "e2")}
        )

        # check next nodes for all current nodes in graph
        self.assertEqual(G.nodes["e1"].get_next_node(), "j1")
        self.assertEqual(G.nodes["e2"].get_next_node(), "j2")
        self.assertEqual(G.nodes["j1"].get_next_node(), "e2")
        self.assertEqual(G.nodes["j2"].get_next_node(), "e2")

        # check degree of incoming and outgoing edges
        self.assertEqual(G.nodes["e1"].degree_incoming(), 0)
        self.assertEqual(G.nodes["e1"].degree_outgoing(), 1)
        self.assertEqual(G.nodes["e2"].degree_incoming(), 2)
        self.assertEqual(G.nodes["e2"].degree_outgoing(), 1)
        self.assertEqual(G.nodes["j1"].degree_incoming(), 1)
        self.assertEqual(G.nodes["j1"].degree_outgoing(), 1)
        self.assertEqual(G.nodes["j2"].degree_incoming(), 1)
        self.assertEqual(G.nodes["j2"].degree_outgoing(), 1)
        ...
        self.assertEqual(G.nodes["e1"].degree_outgoing(), 1)
        self.assertEqual(G.nodes["j1"].degree_incoming(), 1)
        self.assertEqual(G.nodes["j1"].degree_outgoing(), 0)
        self.assertEqual(G.nodes["j2"].degree_incoming(), 0)
        self.assertEqual(G.nodes["j2"].degree_outgoing(), 0)
    ...
    def test_find_cycle_in_graph(
        ...
        self.assertListEqual(cycle, ["e2", "j2"])
        ...
        self.assertCountEqual(matches["e2"], "j2")
        self.assertCountEqual(matches["j2"], "e2")
        self.assertSetEqual(set(G.nodes.keys()), set(["e1", "j1"]))
        self.assertEqual(G.number_of_nodes_in_graph(), 2)
        self.assertSetEqual(G.edges, {("e1", "j1")})
    ...
    def test_find_cycle_in_graph(
        ...
        self.assertSetEqual(G.edges, {("e1", "j1"), ("j1", "e1")})
        # check degree of incoming and outgoing edges
        self.assertEqual(G.nodes["e1"].degree_incoming(), 1)
        self.assertEqual(G.nodes["e1"].degree_outgoing(), 1)
        self.assertEqual(G.nodes["j1"].degree_incoming(), 1)
        self.assertEqual(G.nodes["j1"].degree_outgoing(), 1)
        self.assertEqual(job_queue["e1"], 0)
        self.assertEqual(employee_queue["j1"], 1)
